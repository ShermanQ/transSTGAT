...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        # print("traj_lstm_input_size:",traj_lstm_input_size, "traj_lstm_hidden_size:", traj_lstm_hidden_size)
        # shape(2,32)
        '''  
        '''
        # print("traj_lstm_input_size",traj_lstm_input_size)
        self.nn_trans = nn.Transformer(d_model=1, nhead=1, num_decoder_layers=1,
                                       num_encoder_layers=1)
        # self.traj_lstm_model = nn_trans(traj_lstm_input_size, traj_lstm_hidden_size)
        # self.graph_lstm_model = nn_trans(
    ...
    def forward(
        ...
        for i, input_t in enumerate(
            ...
            # shape():traj_lstm_h_t ,traj_lstm_c_t
            # input_t.squeeze(0).shape : x*2;
            # traj_lstm_h_t.shape & traj_lstm_c_t.shape  x*32
            # print(input_t.squeeze(0).shape[0])
            #device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
            tgt = torch.rand(16 * input_t.squeeze(0).shape[0], 2, 1).cuda()
            #print("input_t.squeeze(0):", input_t.squeeze(0).shape, "tgt.shape:", tgt.shape)
            intem = input_t.squeeze(0).view(input_t.squeeze(0).shape[0], 2, 1).cuda()
            traj_lstm_h_t = self.nn_trans(intem, tgt)
            traj_lstm_h_t = traj_lstm_h_t.view(input_t.squeeze(0).shape[0], -1)
            # print("input_t.squeeze(0):",input_t.squeeze(0).shape)
            # print("traj_lstm_h_t:", traj_lstm_h_t.shape,"traj_lstm_c_t:",traj_lstm_c_t.shape)
